...
#   -> T (periodo)
#
# Metodo: diferencias centrales finitas con delta = DELTA_PX
#   d(f)/d(x) ~ [f(x+d) - f(x-d)] / (2d)
#   sigma_f = sqrt( (df/dx * sigma_x)^2 + (df/dy * sigma_y)^2 )
#           = sqrt( ((f(x+d)-f(x-d))/2)^2 + ((f(y+d)-f(y-d))/2)^2 )
#   ya que sigma_x = sigma_y = d = DELTA_PX  =>  el d se cancela
# ==================================================
def sigma_Lambda_num(mu_s, R_s, xm, ym, xc, yc, beta_o, l_sol):
    """Incertidumbre en Lambda (longitud heliografica) [grados]
    propagada desde +-DELTA_PX pixeles en xm e ym."""
    d = DELTA_PX
    _, L_xp, _, _, _ = solve_mancha_heliografica(mu_s, R_s, xm+d, ym,   xc, yc, beta_o, l_sol)
    _, L_xm, _, _, _ = solve_mancha_heliografica(mu_s, R_s, xm-d, ym,   xc, yc, beta_o, l_sol)
    _, L_yp, _, _, _ = solve_mancha_heliografica(mu_s, R_s, xm,   ym+d, xc, yc, beta_o, l_sol)
    _, L_ym, _, _, _ = solve_mancha_heliografica(mu_s, R_s, xm,   ym-d, xc, yc, beta_o, l_sol)
    # diferencias en grados (angulos pequenos, no hay salto de 360)
    dLx = degrees(L_xp) - degrees(L_xm)
    dLy = degrees(L_yp) - degrees(L_ym)
    return 0.5 * sqrt(dLx**2 + dLy**2)   # grados

def sigma_Phi_num(mu_s, R_s, xm, ym, xc, yc, beta_o, l_sol):
    """Incertidumbre en Phi (latitud heliografica) [grados]
    propagada desde +-DELTA_PX pixeles en xm e ym."""
    d = DELTA_PX
    Ph_xp, _, _, _, _ = solve_mancha_heliografica(mu_s, R_s, xm+d, ym,   xc, yc, beta_o, l_sol)
    Ph_xm, _, _, _, _ = solve_mancha_heliografica(mu_s, R_s, xm-d, ym,   xc, yc, beta_o, l_sol)
    Ph_yp, _, _, _, _ = solve_mancha_heliografica(mu_s, R_s, xm,   ym+d, xc, yc, beta_o, l_sol)
    Ph_ym, _, _, _, _ = solve_mancha_heliografica(mu_s, R_s, xm,   ym-d, xc, yc, beta_o, l_sol)
    dPx = degrees(Ph_xp) - degrees(Ph_xm)
    dPy = degrees(Ph_yp) - degrees(Ph_ym)
    return 0.5 * sqrt(dPx**2 + dPy**2)   # grados
...
def main():
    ...
    while True:
        ...
        if mode == 'a':
            ...
            term_n = tan(PHI_ZERO) / sin(LAMBDA_NORTH) if sin(LAMBDA_NORTH) != 0 else 0
            theta_norte = atan(term_n) - mu_signed - beta_opt
            rho_norte = (cos(PHI_ZERO) * sin(LAMBDA_NORTH)) / cos(theta_norte + mu_signed + beta_opt)
...
def calcular_norte(mu_s, R_s, xc, yc, beta_o):
    import math
    sin_LN = math.sin(LAMBDA_NORTH_W)
    tan_PHI = math.tan(PHI_ZERO_W)
    if sin_LN != 0:
        term_n = tan_PHI / sin_LN
        theta_norte = math.atan(term_n) - mu_s - beta_o
        denom = math.cos(theta_norte + mu_s + beta_o)
        rho_norte = (math.cos(PHI_ZERO_W) * sin_LN) / denom if denom != 0 else 0.0
    else:
        theta_norte = 0.0; rho_norte = 0.0
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["Tabla: Observaciones (Fotos)", "Tabla: Mediciones (Manchas)", "Resultados Calculados", "Animacion Solar", "Errores (+-sigma)"])
            ...
            with st.expander(titulo, expanded=expandido_por_defecto):
                ...
                with col2:
                    ...
                    try:
                        ...
                        xn, yn = calcular_norte(mu_r, R_s, xc, yc, be_r)
...
with tab4:
    ...
    st.info("Solo lectura. La animacion usa Phi y Lambda actuales de la BD (cualquier cambio que guardes se refleja al instante).")
    if not TIENE_PLOTLY:
        st.error("Instala plotly: pip install plotly")
    else:
        conn = get_connection()
        df_anim = pd.read_sql_query("""
        ...
        if df_anim.empty:
            st.info("No hay datos de manchas en la base de datos.")
        else:
            FMTS_A = ['%d-%m-%Y %H:%M','%d/%m/%Y %H:%M','%Y-%m-%d %H:%M:%S','%Y-%m-%d %H:%M','%d-%m-%Y']
            def parse_dt(s):
                for f in FMTS_A:
            ...
            df_anim['dt'] = df_anim['fecha_hora'].apply(parse_dt)
            df_anim = df_anim.dropna(subset=['dt']).reset_index(drop=True)

            if df_anim.empty:
                st.warning("No se pudieron parsear las fechas.")
            else:
                # FILTRO DE PERIODO: agrupar por meses disponibles (evitar
                # animaciones de cientos de dias cuando hay datos de muchos meses)
                df_anim['_periodo'] = df_anim['dt'].dt.strftime('%Y-%m')
                periodos_disp = sorted(df_anim['_periodo'].unique())
                if len(periodos_disp) > 1:
                    st.markdown("**Selecciona el periodo a animar:**")
                    periodos_sel = st.multiselect(
                        "Periodos disponibles (deselecciona los que no quieras)",
                        periodos_disp,
                        default=[periodos_disp[-1]],
                        key="an_periodos",
                        help="Animacion se limitara a las fechas seleccionadas. Limite recomendado: un mes a la vez."
                    )
                    if not periodos_sel:
                        st.warning("Selecciona al menos un periodo para animar.")
                        st.stop()
                    df_anim = df_anim[df_anim['_periodo'].isin(periodos_sel)].reset_index(drop=True)
                    if df_anim.empty:
                        st.warning("No hay datos en el periodo seleccionado.")
                        st.stop()

                t_ref = df_anim['dt'].min()
                df_anim['t_obs'] = (df_anim['dt'] - t_ref).dt.total_seconds() / 86400.0

                # Salvaguarda: limitar duracion maxima de la animacion a 60 dias
                dt_span = df_anim['t_obs'].max() - df_anim['t_obs'].min()
                if dt_span > 60:
                    st.error(
                        "El rango temporal seleccionado es de {{:.0f}} dias. "
                        "Por rendimiento, selecciona MENOS periodos (idealmente uno solo).".format(dt_span)
                    )
                    st.stop()

                LN_deg = math.degrees(LAMBDA_NORTH_W)
                df_anim['cmd0'] = df_anim.apply(
                ...
                def omega_syn(phi_deg):
                    ...
                    return 14.370 - 0.9856 - 2.300 * math.sin(pr)**2

                # ---- CONTROLES UI ----
                c1, c2, c3 = st.columns(3)
                with c1: margen_a = st.number_input("Dias antes",    0, 20, 3, key="an_a")
                with c2: margen_d = st.number_input("Dias despues",  0, 20, 3, key="an_d")
                with c3: paso_h   = st.selectbox  ("Paso (horas)", [6, 12, 24], index=1, key="an_p")

                c4, c5, c6 = st.columns(3)
                with c4: mostrar_traz   = st.checkbox ("Trayectoria", value=True, key="an_tr")
                with c5: mostrar_label  = st.checkbox ("Etiquetas",   value=True, key="an_lb")
                with c6: vista_preset   = st.selectbox("Vista",
                                            ["Frontal", "Polo Norte", "Lateral", "Libre"],
                                            index=0, key="an_v")

                # Selector de mancha en FOCO
                _grupos_para_selector = ["(Todas)"] + [
                    "M" + str(g) for g in sorted(df_anim['id_grupo'].unique())]
                foco_mancha = st.selectbox(
                    "Mancha en foco (resaltada con halo y trayectoria)",
                    _grupos_para_selector, index=0, key="an_foco")
                _id_foco = None
                if foco_mancha != "(Todas)":
                    _id_foco = foco_mancha[1:]   # quita la "M"

                t_start = df_anim['t_obs'].min() - float(margen_a)
                t_end   = df_anim['t_obs'].max() + float(margen_d)
                paso_d  = paso_h / 24.0
                t_steps = []
                t = t_start
                while t <= t_end + 1e-9:
                    t_steps.append(round(t, 4))
                    t += paso_d
                if not t_steps: t_steps = [0.0]

                # ---- COLORES por grupo ----
                COLORES_3D = [
                    '#FF4444', '#44DD44', '#4499FF', '#FFBB00',
                    '#FF44FF', '#00DDDD', '#FF8800', '#AAFF44',
                    '#AA44FF', '#FF4488',
                ]
                grupos_u = sorted(df_anim['id_grupo'].unique())
                cmap_3d  = {{g: COLORES_3D[i % len(COLORES_3D)]
                            for i, g in enumerate(grupos_u)}}

                # ---- ESFERA SOLAR con limb-darkening realista ----
                # Sistema: Y=norte solar, Z=hacia observador
                _nv, _nu = 40, 80
                _Xs, _Ys, _Zs, _Cs = [], [], [], []
                for _iv in range(_nv):
                    _vv = math.pi * (-0.5 + _iv / (_nv - 1))
                    _rx, _ry, _rz, _rc = [], [], [], []
                    for _iu in range(_nu):
                        _uu = 2 * math.pi * _iu / (_nu - 1)
                        _xx = math.cos(_vv) * math.sin(_uu)
                        _yy = math.sin(_vv)
                        _zz = math.cos(_vv) * math.cos(_uu)
                        _rx.append(_xx); _ry.append(_yy); _rz.append(_zz)
                        # mu = cos(angulo desde linea de vision) = z para esfera unidad
                        _mu = max(0.0, _zz)
                        # ley de Eddington-Barbier: I(mu) ~ 0.4 + 0.6*mu
                        _rc.append(0.4 + 0.6 * _mu)
                    _Xs.append(_rx); _Ys.append(_ry)
                    _Zs.append(_rz); _Cs.append(_rc)

                sphere_tr = go.Surface(
                    x=_Xs, y=_Ys, z=_Zs,
                    surfacecolor=_Cs,
                    colorscale=[
                        [0.0, '#2a0500'], [0.20, '#7a1a00'],
                        [0.45, '#cc5500'], [0.70, '#ff9930'],
                        [0.90, '#ffd070'], [1.0,  '#fff5c0'],
                    ],
                    showscale=False, opacity=1.0,
                    hoverinfo='skip', showlegend=False)

                # ---- Grid heliografico ----
                def _par3(phi_d, n=120):
                    pr  = math.radians(phi_d)
                    lns = [2*math.pi*i/(n-1) - math.pi for i in range(n)]
                    return ([1.006*math.cos(pr)*math.sin(lr) for lr in lns],
                            [1.006*math.sin(pr)]*n,
                            [1.006*math.cos(pr)*math.cos(lr) for lr in lns])

                def _mer3(lon_d, n=100):
                    lr  = math.radians(lon_d)
                    phs = [math.pi*(-0.5 + i/(n-1)) for i in range(n)]
                    return ([1.006*math.cos(p)*math.sin(lr) for p in phs],
                            [1.006*math.sin(p) for p in phs],
                            [1.006*math.cos(p)*math.cos(lr) for p in phs])

                grid_3d = []
                # Paralelos cada 30 deg: ecuador BLANCO, resto muy tenue
                for _phi_g in [60, 30, 0, -30, -60]:
                    _gx, _gy, _gz = _par3(_phi_g)
                    if _phi_g == 0:
                        _col = 'rgba(255,255,255,0.92)'; _lw = 3
                    else:
                        _col = 'rgba(255,230,80,0.13)'; _lw = 1
                    grid_3d.append(go.Scatter3d(x=_gx, y=_gy, z=_gz, mode='lines',
                        line=dict(color=_col, width=_lw),
                        showlegend=False, hoverinfo='skip'))
                # Meridianos cada 30 deg: central (lon 0) CIAN
                for _lon_g in range(-150, 181, 30):
                    _mx, _my, _mz = _mer3(_lon_g)
                    if _lon_g == 0:
                        _col = 'rgba(0,255,255,0.95)'; _lw = 4
                    else:
                        _col = 'rgba(255,230,80,0.10)'; _lw = 1
                    grid_3d.append(go.Scatter3d(x=_mx, y=_my, z=_mz, mode='lines',
                        line=dict(color=_col, width=_lw),
                        showlegend=False, hoverinfo='skip'))

                # ---- Polos ----
                poles_tr = [
                    go.Scatter3d(x=[0], y=[1.18], z=[0], mode='markers+text',
                        marker=dict(color='cyan', size=6),
                        text=['N'], textposition='top center',
                        textfont=dict(color='cyan', size=18, family='Arial Black'),
                        showlegend=False, hoverinfo='skip'),
                    go.Scatter3d(x=[0], y=[-1.18], z=[0], mode='markers+text',
                        marker=dict(color='cyan', size=6),
                ...
                # ---- DATOS POR MANCHA (UN solo registro por mancha) ----
                # Cada mancha se representa con su PRIMERA observacion como referencia.
                # En cada frame t_val, la posicion se calcula propagando la rotacion
                # desde esa referencia: cmd(t) = cmd0 - omega_syn(phi) * (t - t_obs0)
                manchas_info = []
                for _grp in grupos_u:
                    _df_g = df_anim[df_anim['id_grupo'] == _grp].sort_values('dt').reset_index(drop=True)
                    if len(_df_g) == 0:
                        continue
                    _row0 = _df_g.iloc[0]
                    _phi_med = float(_df_g['latitud_phi'].mean())
                    manchas_info.append(dict(
                        grp        = _grp,
                        phi        = _phi_med,
                        cmd0       = float(_row0['cmd0']),
                        tobs0      = float(_row0['t_obs']),
                        t_inicio   = float(_df_g['t_obs'].min()),
                        t_fin      = float(_df_g['t_obs'].max()),
                        n_obs      = len(_df_g),
                        primera    = str(_row0['fecha_hora'])[:16],
                        ultima     = str(_df_g.iloc[-1]['fecha_hora'])[:16],
                    ))

                # ---- Trayectoria SOLO de la mancha en foco (o todas si "Todas") ----
                def _trail_for_group(_grp):
                    _df_g = df_anim[df_anim['id_grupo'] == _grp].sort_values('dt')
                    if len(_df_g) < 2:
                        return None
                    _xs, _ys, _zs = [], [], []
                    for _, _row in _df_g.iterrows():
                        _phi = float(_row['latitud_phi'])
                        _c0  = float(_row['cmd0'])
                        _pr  = math.radians(_phi)
                        _cr  = math.radians(_c0)
                        _r   = 1.022
                        _xs.append(_r * math.cos(_pr) * math.sin(_cr))
                        _ys.append(_r * math.sin(_pr))
                        _zs.append(_r * math.cos(_pr) * math.cos(_cr))
                    return go.Scatter3d(
                        x=_xs, y=_ys, z=_zs, mode='lines+markers',
                        line=dict(color=cmap_3d[_grp], width=5, dash='dot'),
                        marker=dict(color=cmap_3d[_grp], size=5,
                                    line=dict(color='white', width=1)),
                        name='Traza M' + str(_grp),
                        showlegend=False,
                        hoverinfo='skip')
                ...
                if mostrar_traz:
                    if _id_foco is None:
                        # Todas las trayectorias (modo (Todas))
                        for _grp in grupos_u:
                            _tr = _trail_for_group(_grp)
                            if _tr is not None:
                                trails_tr.append(_tr)
                    else:
                        # Solo la trayectoria de la mancha en foco
                        for _grp in grupos_u:
                            if str(_grp) == _id_foco:
                                _tr = _trail_for_group(_grp)
                                if _tr is not None:
                                    trails_tr.append(_tr)

                static_tr = [sphere_tr] + grid_3d + poles_tr + trails_tr
                n_static  = len(static_tr)

                # ---- Manchas dinamicas: UNA por id_grupo, foco resaltado con halo ----
                def build_spots(t_val):
                    out = []
                    # HALO de la mancha en foco (siempre la primera trace dinamica)
                    _halo_x, _halo_y, _halo_z, _halo_color = [], [], [], '#ffffff'
                    if _id_foco is not None:
                        for info in manchas_info:
                            if str(info['grp']) == _id_foco:
                                _phi = info['phi']
                                _ct  = info['cmd0'] - omega_syn(_phi) * (t_val - info['tobs0'])
                                _ct  = ((_ct + 180) % 360) - 180
                                _pr  = math.radians(_phi); _cr = math.radians(_ct)
                                _r   = 1.055
                                _halo_x = [_r * math.cos(_pr) * math.sin(_cr)]
                                _halo_y = [_r * math.sin(_pr)]
                                _halo_z = [_r * math.cos(_pr) * math.cos(_cr)]
                                _halo_color = cmap_3d[info['grp']]
                                break
                    # La trace del halo siempre existe (vacia si no hay foco)
                    out.append(go.Scatter3d(
                        x=_halo_x, y=_halo_y, z=_halo_z, mode='markers',
                        marker=dict(
                            color=_halo_color, size=60, symbol='circle',
                            opacity=0.18,
                            line=dict(color=_halo_color, width=2)),
                        name='HALO',
                        showlegend=False,
                        hoverinfo='skip'))
                    for info in manchas_info:
                        ...
                        _ct  = ((_ct + 180) % 360) - 180
                        _pr  = math.radians(_phi)
                        _cr  = math.radians(_ct)
                        _r   = 1.045
                        _x   = _r * math.cos(_pr) * math.sin(_cr)
                        _y   = _r * math.sin(_pr)
                        _z   = _r * math.cos(_pr) * math.cos(_cr)
                        _visible = abs(_ct) < 90
                        _es_foco = (_id_foco is not None and str(info['grp']) == _id_foco)
                        _hay_foco = (_id_foco is not None)
                        # Determinar tamano y opacidad segun estado
                        if _es_foco:
                            _size_main = 28
                            _opacity   = 1.0
                            _label_sz  = 13
                        elif _hay_foco:
                            # Hay foco pero esta mancha NO es la elegida -> atenuada
                            _size_main = 10
                            _opacity   = 0.30 if _visible else 0.12
                            _label_sz  = 9
                        else:
                            # No hay foco -> tamano normal segun visibilidad
                            _size_main = 16
                            _opacity   = 1.0 if _visible else 0.35
                            _label_sz  = 11
                        _ht = (
                            "<b>M" + str(info['grp']) + "</b>" +
                            "<br>Phi = " + str(round(_phi, 2)) + " deg" +
                            "<br>CMD = " + str(round(_ct, 2)) + " deg" +
                            "<br>Obs: " + str(info['n_obs']) +
                            "<br>" + ("[VISIBLE]" if _visible else "[lado oculto]")
                        )
                        _mode = 'markers+text' if mostrar_label else 'markers'
                        out.append(go.Scatter3d(
                            x=[_x], y=[_y], z=[_z], mode=_mode,
                            marker=dict(
                                color=cmap_3d[info['grp']], size=_size_main,
                                line=dict(color='black', width=2),
                                symbol='circle',
                                opacity=_opacity),
                            text=['M' + str(info['grp'])],
                            textposition='middle center',
                            textfont=dict(color='black', size=_label_sz, family='Arial Black'),
                            name='M' + str(info['grp']) + ' (Phi=' + str(round(_phi,1)) + ' deg)',
                            hovertext=[_ht], hoverinfo='text',
                            showlegend=True))
                    return out

                # idx_spots incluye: halo (1) + una por mancha
                idx_spots = list(range(n_static, n_static + 1 + len(manchas_info)))

                # ---- Camara presets ----
                CAM_PRESETS = {{
                    "Frontal":    dict(up=dict(x=0,y=1,z=0), center=dict(x=0,y=0,z=0), eye=dict(x=0,    y=0.25, z=2.5)),
                    "Polo Norte": dict(up=dict(x=0,y=0,z=1), center=dict(x=0,y=0,z=0), eye=dict(x=0,    y=2.6,  z=0.3)),
                    "Lateral":    dict(up=dict(x=0,y=1,z=0), center=dict(x=0,y=0,z=0), eye=dict(x=2.5,  y=0.25, z=0  )),
                    "Libre":      dict(up=dict(x=0,y=1,z=0), center=dict(x=0,y=0,z=0), eye=dict(x=0.05, y=0.4,  z=2.2)),
                }}
                cam_cfg = CAM_PRESETS[vista_preset]

                frames_list = []
                for t_val in t_steps:
                    fecha_f = (t_ref + pd.Timedelta(days=t_val)).strftime('%d-%m-%Y  %H:%M')
                    frames_list.append(go.Frame(
                        data=build_spots(t_val),
                        traces=idx_spots,
                        name=str(t_val),
                        layout=go.Layout(
                            title_text='Rotacion Solar 3D  ·  ' + fecha_f)))

                slider_steps = [
                    dict(method='animate',
                         args=[[str(tv)], dict(mode='immediate',
                             frame=dict(duration=250, redraw=True),
                             transition=dict(duration=0))],
                         label=(t_ref + pd.Timedelta(days=tv)).strftime('%d/%m'))
                    for tv in t_steps]

                fig = go.Figure(
                    data=static_tr + build_spots(t_steps[0]),
                    frames=frames_list,
                    layout=go.Layout(
                        title=dict(
                            text='Rotacion Solar 3D  ·  ' + t_ref.strftime('%d-%m-%Y'),
                        ...
                        scene=dict(
                            ...
                            camera=cam_cfg,
                            aspectmode='cube'),
                        paper_bgcolor='#02010a',
                        font=dict(color='white'),
                        width=820, height=740,
                        margin=dict(l=0, r=170, t=70, b=110),
                        legend=dict(
                            x=1.02, y=0.88,
                            bgcolor='rgba(10,10,40,0.85)',
                ...
                st.caption(
                    "**Meridiano central** cian · **Ecuador** blanco · "
                    "**Etiquetas M{{x}}** = identificador de mancha en la base de datos. "
                    "Selecciona una mancha en el desplegable superior para resaltarla con halo y dibujar solo su trayectoria. "
                    "Cuando la mancha esta en el lado oculto del Sol (|CMD| > 90°) se atenua al 35%. "
                    "Arrastra para rotar · rueda para zoom · cambia la vista con el selector superior."
                )
